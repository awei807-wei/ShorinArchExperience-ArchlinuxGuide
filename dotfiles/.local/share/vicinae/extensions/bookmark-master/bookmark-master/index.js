...
function Command() {
    ...
    const openAndFocus = (url) => {
        exec(`thorium-browser "${url}"`, (err) => {
            if (err) return;
            // 延迟 150ms 等待窗口拉起，然后强制聚焦
            setTimeout(() => {
                exec("niri msg action focus-window --app-id thorium-browser");
            }, 150);
        });
    };
        ...
        filteredItems.map(it => React.createElement(List.Item, {
            ...
            actions: React.createElement(
                ...
                React.createElement(Action, {
                    title: "在 Thorium 中打开并聚焦",
                    icon: Icon.Globe,
                    onAction: () => openAndFocus(it.url)
                }),
                React.createElement(Action, {
                    title: "编辑备注",
                    icon: Icon.Pencil,
