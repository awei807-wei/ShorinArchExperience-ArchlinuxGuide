...
ContextContent {
    ...
    readonly property var currentWorkspace: niriModel ? niriModel.workspaceForOutput(screenName) : null
    readonly property int activeWorkspace: currentWorkspace ? currentWorkspace.idx : 1
    readonly property int rangeStart: Math.floor((activeWorkspace - 1) / 5) * 5 + 1
    readonly property int rangeEnd: rangeStart + 4
    readonly property var occupiedWorkspaces: niriModel ? niriModel.occupiedIndexes(screenName) : []
    ...
    WorkspaceStrip {
        anchors.fill: parent
        labelText: "RANGE"
        valueText: niriContext.pad2(niriContext.rangeStart) + "–" + niriContext.pad2(niriContext.rangeEnd)
        subText: niriContext.niriModel && niriContext.niriModel.errorMessage !== "" ? niriContext.niriModel.errorMessage : "WS " + niriContext.pad2(niriContext.activeWorkspace)
