...
## 产品表面
桌面常驻界面由顶部 Bar、临时通知与统一右侧子面板组成。Bar 只承载高频上下文、时间和系统摘要；控制中心与通知历史作为同一子面板的两个页面承载详情和操作，避免把顶栏做成信息墙。独立锁屏沿用同一暗色层级与主题色，但以身份确认和单一凭据控件为中心，不复制 Bar 的连续轮廓。
...
- 密码输入与 `92×40px` 解锁按钮必须组合在同一个 `360×54px`、圆角 `16px` 的凭据框内。空密码在已聚焦时仍显示“输入密码”，只用透明度区分焦点权重；PAM 活跃时禁用输入和提交。
- 凭据框下方始终保留 `22px` 状态槽。验证中显示 spinner 与状态文字；认证失败使用错误色边框和水平位移反馈，不旋转控件、不改变认证区高度；减弱动效时直接切换状态。
- 右上角使用 `96×44px` 统一操作胶囊，距顶部 `20px`、距右侧 `24px`。power / eye / eye-off 由 `LockGlyph` 以 Canvas 固定坐标绘制，图标尺寸 `18×18px`；锁屏操作图标禁止使用 Nerd Font、普通 Unicode 或其他字体字符。
...
## 动效策略
常规交互反馈限定为 `120–180ms` 的颜色与轻量过渡。右侧子面板的连接几何只允许一条 `300ms OutCubic` 时间轴：右岛宽度、面板 viewport 宽高与退场 mask 读取同一 `rightPanelProgress`，不得再叠加 `Behavior` 或独立宽高缓动。前 `10%` 只让右岛响应，面板主体随后从 `54px` 安全高度淡入；内容仅由同一进度派生，并从 `25%` 后淡入、上移 `8px`。页面切换只做同时开始的 `150/200ms` 交叉淡入淡出、方向相反的 `28px` 位移和一次 `0.985→1` 卡片缩放，退出与进入之间不留空白帧，外壳不参与；History 来源切换只允许列表先用 `60ms` 淡到 `0.35`、换模型后用 `120ms` 淡回，不重播逐卡飞入。面板窗口常驻映射，关闭到端点后保留 `20ms` 再撤掉输入区域；禁止随开合映射/卸载窗口或用 `visible` 卸载内容，内容首帧重建实测造成 `40–90ms` 单帧停顿。中岛子面板由 `280/220ms OutQuad` 单进度驱动 Bar 缺口与内容裁剪，内容透明度按 `smoothstep(0.30, 0.90)` 随裁剪显露并上抬 `8px`，时钟文字在前 `30%` 淡出，Home / System / Tasks 以 `150ms` 交叉淡入切换。启动后 `1500ms` 内两个面板以 `0.001` 透明度预渲染内容。临时通知卡片退场时同步收拢高度，下方卡片只由 Column 的 `move` 过渡上移。`QUICKSHELL_REDUCE_MOTION=1` 时直接到端点。
