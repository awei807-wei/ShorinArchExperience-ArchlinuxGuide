...
- [2026-08-28] Cava 频谱属于整个右岛的材质层，不属于 Metrics 内容层：由 `Bar` 按活动右岛颈宽铺设在 `BarContour` 与交互组件之间，32 根 `3.6px` 柱体等距分布。静止和活动态都保持低对比，Metrics、Tray 与 Power 的输入和 hover 反馈始终位于其上。
