...
## 目标与范围
本次同时闭合三个相互独立但需要统一验收的行为：修正托盘点击后的持久灰框与动态宽度、把持久通知历史来源计数接入每应用排序/角标、从 Git 历史恢复 niri 快捷键指向的独立锁屏。Bar 继续保持最多 3 个直接应用图标和既有复合入口；锁屏只做定向恢复，不回退整个工作区。
...
- `notification-history.py` 按裁剪后的持久历史归一化聚合 `(desktopEntry, appName)` 来源计数；`NotificationHistoryStore.qml` 在 `append/count/list/clear` 成功后更新 `sourceCounts`，经 `shell.qml -> Bar.qml -> SystemIsland.qml -> TrayIsland.qml` 贯穿。`notificationGroups` 仅负责临时弹窗。
- 持久 `sourceCounts` 经 `shell.qml`、`Bar.qml`、`SystemIsland.qml` 传入 `TrayIsland.qml`；临时分组不参与角标，历史面板继续消费 `historyCount`。
...
- “来源计数”：磁盘通知历史中按规范化 `(desktopEntry, appName)` 聚合的保留记录数，不定义为未读数。
...
- 持久来源、Fcitx/VCP/飞书/QQ 精确与受限匹配、歧义拒绝、排序示例和 `TrayItem.notificationCount` 委托角标全部由测试证明；清空历史后顺序与角标恢复，临时分组生命周期独立。
...
- `components/TrayNotificationModel.js`：持久来源计数的精确匹配、受限 QQ 映射与稳定排序。
- `components/NotificationHistoryStore.qml`：历史总数、持久 `sourceCounts` 和串行存储队列。
