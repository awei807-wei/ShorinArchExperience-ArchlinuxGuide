...
## [未发布] - 2026-08-08
### 新增
- **[Tray]**: 新增基于磁盘通知来源计数的托盘稳定排序与每应用危险色角标，计数相同时保持 SystemTray 注册顺序。
...
### 验证
- 自动门禁通过：Python 通知历史 11 项、offscreen 托盘/存储/布局检查、`qmllint lockscreen/shell.qml` 与 `git diff --check`。
- 真实托盘点击、右键菜单、键盘焦点以及错误密码/正确密码解锁仍待人工验收。
