...
### 修复
- **[右侧子面板]**: 修复 History 切换到 Control 时，线程化外壳 Canvas 随页面高度动画重建纹理而出现顶部透明帧、露出壁纸的问题。两页改为共用固定高度，外壳由单个最终尺寸 Canvas 稳定绘制，跨页不再触发几何或纹理尺寸变化。
- **[右侧子面板]**: 面板窗口上移 `15px`，让主体顶边与右岛底边共用接缝；flare 仅覆盖视觉接缝且不抢占 Bar 点击区域。
- **[右侧子面板]**: 固定外窗尺寸和锚点；内部 sizer 只负责开合裁剪，页面内容和外壳按共享目标高度排版。外壳 Canvas 使用固定最终尺寸线程化渲染，内容只做透明度、位移和一次轻量卡片缩放，减少 4K/1.5× 下的主线程掉帧。
