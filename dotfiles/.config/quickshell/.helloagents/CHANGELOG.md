...
- **[右侧子面板]**: 开启动画拆为颈部、横向、纵向和内容四个时间段；关闭按相反层级执行。页面保持常驻，在固定外壳内通过交叉淡入淡出、方向相反的 `28px` 位移和 `0.985→1` 轻量缩放完成整页卡片切换。
...
### 修复
- **[右侧子面板]**: 修复横向开合时活动裁剪线穿过固定左上圆角、造成左边缘短暂挤压的问题。主体改为随 sizer 当前宽高同步生长的原生圆角矩形，颈部连接弧收敛为固定小尺寸即时 Canvas；现有 `40/95/180ms` 分阶段时序保持不变。
...
- **[右侧子面板]**: 固定外窗尺寸和锚点；内部 sizer 驱动轻量原生外壳并裁剪最终尺寸内容，页面内容按共享目标高度排版，只做透明度、位移和一次轻量卡片缩放，减少 4K/1.5× 下的布局与纹理负担。
...
### 验证
- 右面板动画门禁新增 `16/17/34/52px` 高度、`320/322px` 宽度临界圆角，以及关闭态、横向领先、宽高并行、内容进入和关闭阶段的活动轮廓贴边断言；高对比离屏逐帧人工对照确认左侧圆角连续跟随边缘，最终展开态轮廓保持不变。
