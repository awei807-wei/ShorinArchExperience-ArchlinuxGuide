...
## 目标与范围
本次同时闭合三个相互独立但需要统一验收的行为：修正托盘点击后的持久灰框与动态宽度、把现有磁盘通知历史聚合为每应用排序/角标数据、从 Git 历史恢复 niri 快捷键指向的独立锁屏。Bar 继续保持最多 3 个直接应用图标和既有复合入口；锁屏只做定向恢复，不回退整个工作区。
...
- `notification-history.py` 在每次成功响应中，从已经完成清洗和双上限裁剪的历史记录按 `(desktopEntry, appName)` 分桶，返回稳定的 `sourceCounts` 数组；空历史返回空数组，清空后计数同步归零。
- `NotificationHistoryStore.qml` 只常驻总历史数、轻量来源计数和串行队列，不加载完整历史作为排序数据。`sourceCounts` 经 `shell.qml`、`Bar.qml`、`SystemIsland.qml` 传入 `TrayIsland.qml`。
...
- 磁盘历史成功响应稳定携带 `sourceCounts`；匹配规则、歧义拒绝和排序示例全部由测试证明，清空后顺序与角标恢复。
...
- `scripts/notification-history.py`：裁剪后来源计数聚合与响应字段。
- `components/NotificationHistoryStore.qml`：轻量 `sourceCounts` 状态与更新信号。
...
- 存储脚本新增字段可能破坏旧 QML：保持现有 `ok/count/notifications/warning` 字段，QML 对缺失 `sourceCounts` 按空数组兼容。
