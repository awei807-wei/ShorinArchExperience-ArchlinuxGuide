...
- [√] 任务2（AFK）：交付磁盘历史的每来源计数（依赖：无；涉及文件：`scripts/notification-history.py`、`scripts/test-notification-history.py`、`components/NotificationHistoryStore.qml`、`notification-store-check.qml`；预期变更：成功响应新增裁剪后 `sourceCounts`，Store 常驻轻量计数并在 append/count/list/clear 后同步；完成标准：同源聚合、双上限裁剪、损坏恢复、清空和旧字段兼容均有测试；验证方式：`python3 scripts/test-notification-history.py` 与 Quickshell 运行 `notification-store-check.qml`）。
- [√] 任务3（AFK）：交付通知驱动的稳定排序与每应用角标（依赖：任务1、任务2；涉及文件：`shell.qml`、`Bar.qml`、`components/SystemIsland.qml`、`components/TrayIsland.qml`、`tray-state-check.qml`；预期变更：来源计数贯穿、两阶段唯一精确匹配、计数降序/注册索引升序、危险色数字角标与清零复位；完成标准：`1/2/3/4 -> 4/1/2/3 -> 3/4/1/2`、歧义拒绝、未知来源、同数稳定和清空复位全部通过；验证方式：扩展 `tray-state-check.qml`、通知注入与实际 Bar 视觉检查）。
...
## 进度
- 通知数据链研究已完成：已确认磁盘历史唯一来源、`sourceCounts` 响应结构、唯一匹配规则、排序规则与贯穿路径。
- 托盘布局与锁屏恢复由独立子代理并行施工；主代理负责方案协调、通知实现、集成与最终验证。
- 自动门禁已通过：`python3 scripts/test-notification-history.py`（11 项）、offscreen `tray-state-check.qml`、`notification-store-check.qml`、`bar-layout-check.qml`、`qmllint lockscreen/shell.qml`、`git diff --check`。
