...
- [√] 任务2（AFK）：纠正通知来源数据语义（依赖：无；涉及文件：`scripts/notification-history.py`、`scripts/test-notification-history.py`、`components/NotificationHistoryStore.qml`、`notification-store-check.qml`；预期变更：删除磁盘历史 `sourceCounts` 聚合与 Store 属性，保留历史总数/列表/清空/损坏恢复；完成标准：历史响应不再提供来源计数且 10 项 Python 回归通过；验证方式：`python3 scripts/test-notification-history.py` 与 Quickshell 运行 `notification-store-check.qml`）。
- [√] 任务3（AFK）：交付活动通知驱动的稳定排序与每应用角标（依赖：任务1、任务2；涉及文件：`shell.qml`、`Bar.qml`、`components/SystemIsland.qml`、`components/TrayIsland.qml`、`components/TrayNotificationModel.js`、`tray-state-check.qml`；预期变更：从 `notificationGroups` 派生来源计数并贯穿、两阶段唯一精确匹配、Fcitx/Chrome/Lark fixture、计数降序/注册索引升序、危险色数字角标与关闭复位；完成标准：`1/2/3/4 -> 4/1/2/3 -> 3/4/1/2`、歧义拒绝、未知来源、同数稳定和实际 TrayItem 委托角标全部通过；验证方式：扩展 `tray-state-check.qml`、通知注入与实际 Bar 视觉检查）。
...
- 自动门禁已通过：`python3 scripts/test-notification-history.py`（10 项）、offscreen `tray-interaction-check.qml`、`tray-state-check.qml`、`notification-store-check.qml`、`bar-layout-check.qml`、托盘聚焦 helper fixture、`qmllint lockscreen/shell.qml`、`git diff --check`。
